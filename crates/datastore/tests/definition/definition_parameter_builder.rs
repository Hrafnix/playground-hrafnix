...
fn test_invalid_parameter_keys() {
    ...
    let error = res.expect_err("key without prefix should fail");
    assert_eq!(
...
fn test_parameter_object_definition_inherit() {
    ...
    let keys: Vec<_> = child_def.keys().map(ParameterKey::as_str).collect();
...
fn test_parameter_object_definition_launder() {
    ...
    assert!(laundered.contains("p_p7"));
    assert!(laundered.contains("p_p8"));
    ...
    assert!(store.contains("p_p7"));
    assert!(store.contains("p_p8"));
